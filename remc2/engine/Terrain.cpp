...
building_str building_F2CD0x[2800]; // 233cd0//4802 //4816 // size has to be 2 * (7^4 + 7^3 + 7^2 + 7^1) = 2 * (2401 + 343 + 49 + 7) = 2*2800 as seen in sub_462A0_orig
//0-terrain type,1-terrain rotation
		...
		for (int j = 0; j < 7; j++)
		{
			for (int k = 0; k < 7; k++)
			{
				for (int l = 0; l < 7; l++)
				{
					...
					if (*actBufPos)
					{
						building_F2CD0x[index].terrainTileType = actBufPos[1];
						building_F2CD0x[index].rotationTile = actBufPos[13];
					}
					else
					{
						building_F2CD0x[index].terrainTileType = 1;
						building_F2CD0x[index].rotationTile = 0;
					}
					index++;
				}
			}
	...
	for (int indexY = yAdd; indexY > 0; indexY--)
	{
		for (int indexX = xAdd; indexX > 0; indexX--)
		{
			if (mapTerrainType_10B4E0[tempAxis.word] == 1)
			{
				...
				mapTerrainType_10B4E0[tempAxis.word] = building_F2CD0x[terModIndex].terrainTileType;
				if (building_F2CD0x[terModIndex].terrainTileType >= 8u)
				{
					nextAngle = building_F2CD0x[terModIndex].rotationTile + (mapAngle_13B4E0[tempAxis.word] & 0x87);
				}
				else
				{
					rand2_17B4E0 = 9377 * rand2_17B4E0 + 9439;
